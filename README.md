...
2. **选择平台并创建草稿**：同一区域显示 Ego 登录状态和草稿状态；点击一次后，Harness AI 生成并冻结平台字段，Ego Browser 上传、填表和保存。只有通过远端回读的平台才显示草稿成功；只达到投稿页 `READY` 的平台会显示“页面已备，尚未远端保存”。
...
当前可选择的页面适配器为：B站、抖音、小红书、视频号、快手（视频），以及微信公众号、百家号（文章 + 封面）。B站、抖音和快手已完成真实“远端保存 + id 回读”；微信公众号、百家号、小红书和视频号的运行接线已实现并有本地契约测试，但仍待逐平台真实账号回归。小红书和视频号即使运行成功也只写入 `READY` 中间态，不会伪造远端草稿；微信公众号与百家号可在同一次图文任务中并发运行并分别记录结果。其余十四个图文平台已有隐藏的 `local-tested` Adapter，但真实账号保存与 ID/标题回读完成前不能勾选。
...
安装后在 Harness 设置页选择内容目录，并确认 `ego-browser` 可用。B站、抖音、小红书、视频号和快手的视频草稿还需要 `video-publisher` Skill；微信公众号和百家号图文适配器已经内置。
