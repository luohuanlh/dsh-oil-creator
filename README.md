...
当前可选择的页面适配器为：B站、抖音、小红书、视频号、快手（视频），以及微信公众号、百家号、企鹅号、知乎、雪球号、东方财富号、微博、顶端新闻、头条号、维科网和小红书图文笔记（文章 + 封面）。十个图文平台已完成真实远端保存与 ID/标题回读；小红书图文笔记按官网真实边界保存到 Ego 当前浏览器的 IndexedDB，并标记为 `local-verified`，不会伪装成远端 ID。小红书视频和视频号仍只写入 `READY`。其余七个图文平台保留 `local-tested` Adapter。
...
安装后在 Harness 设置页选择内容目录，并确认 `ego-browser` 可用。B站、抖音、小红书、视频号和快手的视频草稿还需要 `video-publisher` Skill；十个远端验证与一个浏览器本地验证的图文适配器均已内置。
