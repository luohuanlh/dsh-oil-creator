...
当前可选择的页面适配器为：B站、抖音、小红书、视频号（视频），以及微信公众号（文章 + 封面）。B站和抖音已完成真实“远端保存 + id 回读”；微信公众号、小红书和视频号的运行接线已实现并有本地契约测试，但仍待逐平台真实账号回归。小红书和视频号即使运行成功也只写入 `READY` 中间态，不会伪造远端草稿。其他平台只能绑定账号。
