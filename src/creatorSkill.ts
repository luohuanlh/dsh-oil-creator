...
- B站、抖音和快手视频，以及微信公众号、百家号、知乎、雪球号、东方财富号、微博和顶端新闻图文已通过真实远端保存与 ID 回读，可使用独立任务空间并发运行、逐平台记录结果。小红书和视频号当前只准备已验证的投稿页，并明确返回“页面已备”；通过各自的远端保存回归前，不得报告草稿成功。其他平台只有真实适配器通过回归后才能使用。
