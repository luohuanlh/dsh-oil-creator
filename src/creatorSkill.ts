...
- B站视频已通过真实远端保存与 id 回读。微信公众号图文已实现并通过模拟回归，但完成真实账号回归前不得称为远端已验证。抖音、小红书和视频号当前只准备已验证的投稿页，并明确返回“页面已备”；通过各自的远端保存回归前，不得报告草稿成功。其他平台只有真实适配器通过回归后才能使用。
...
- 设置页环境状态只包含内容目录、自动发布、公众号图文和 Ego Browser。
